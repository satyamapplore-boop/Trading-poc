...
from alternative_data_signal import AlternativeDataAggregator, WEIGHTS
...
# ─────────────────────────────────────────────────────────────────────────────
# Alternative Data Aggregator — singleton in session state
# ─────────────────────────────────────────────────────────────────────────────
if "alt_aggregator" not in st.session_state:
    st.session_state.alt_aggregator = AlternativeDataAggregator()
if "alt_data" not in st.session_state:
    st.session_state.alt_data = None   # populated by render_intelligence_feed
...
def render_header():
    ...
    with hc3:
        wa_dot  = "🟢" if cfg.whale_alert_active  else "🔴"
        cp_dot  = "🟢" if cfg.cryptopanic_active  else "🔴"
        ai_dot  = "🟢" if cfg.claude_active        else "🟡"
        st.markdown(
            f"<div style='text-align:right;padding-top:10px;'>"
            f"<div style='font-size:11px;color:#848e9c;margin-bottom:4px;'>🕐 {datetime.now().strftime('%H:%M:%S IST')}</div>"
            f"<div style='font-size:10px;color:#5a6370;line-height:1.6;'>"
            f"{wa_dot} Whale Alert &nbsp; {cp_dot} CryptoPanic &nbsp; {ai_dot} Claude AI"
            f"</div></div>",
            unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────────────────────────────
# INTELLIGENCE FEED  — runs every 60 s, async, non-blocking
# ─────────────────────────────────────────────────────────────────────────────
@st.fragment(run_every=60)
def render_intelligence_feed():
    """Fetches whale + news data via Claude every 60 s and renders the live feed."""
    agg = st.session_state.alt_aggregator
    try:
        result = asyncio.run(agg.get_signal())
        st.session_state.alt_data = result
    except RuntimeError:
        # A running event loop exists (rare in Streamlit); use cached data
        result = st.session_state.alt_data
    except Exception:
        result = st.session_state.alt_data
    ...
    whale_feed = result.get("whale_feed", [])
    news_feed  = result.get("news_feed",  [])
    reasoning  = result.get("reasoning",  "—")
    signal     = result.get("alt_signal", "HOLD")
    confidence = result.get("alt_confidence", 0.5)

    sig_color = {"BUY": "#0ecb81", "SELL": "#f6465d"}.get(signal, "#f0b429")
    sig_bg    = {"BUY": "rgba(14,203,129,.08)", "SELL": "rgba(246,70,93,.08)"}.get(signal, "rgba(240,180,41,.08)")

    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown(
        "<div style='font-size:1rem;font-weight:700;color:#fff;margin-bottom:12px;letter-spacing:.02em;'>"
        "🔬 Live Intelligence Feed <span style='font-size:11px;color:#848e9c;font-weight:400;'>"
        "· refreshes every 60 s</span></div>",
        unsafe_allow_html=True,
    )

    ic1, ic2, ic3 = st.columns([1.2, 1.5, 1.3])

    # ── Column 1: Whale Movements ─────────────────────────────────────────────
    with ic1:
        st.markdown(
            "<div class='card-title' style='font-size:12px;'>🐋 Last 5 BTC Whale Movements</div>",
            unsafe_allow_html=True,
        )
        if whale_feed:
            whale_html = ""
            for w in whale_feed:
                d = w.get("direction", "neutral")
                arrow  = "▲" if d == "bullish" else ("▼" if d == "bearish" else "→")
                d_col  = "#0ecb81" if d == "bullish" else ("#f6465d" if d == "bearish" else "#848e9c")
                d_badge = (
                    f"<span style='background:{'rgba(14,203,129,.15)' if d=='bullish' else 'rgba(246,70,93,.15)' if d=='bearish' else 'rgba(132,142,156,.1)'};"
                    f"color:{d_col};padding:1px 6px;border-radius:4px;font-size:10px;font-weight:700;'>"
                    f"{d.upper()}</span>"
                )
                usd = w.get("amount_usd", 0)
                usd_str = f"${usd/1e6:.2f}M" if usd >= 1_000_000 else f"${usd/1e3:.0f}K"
                label = w.get("label", "—")[:38]
                time_str = w.get("time", "—")
                whale_html += (
                    f"<div style='padding:7px 0;border-bottom:1px solid #1a1f26;'>"
                    f"<div style='display:flex;justify-content:space-between;align-items:center;'>"
                    f"<span style='color:{d_col};font-size:13px;font-weight:800;margin-right:6px;'>{arrow}</span>"
                    f"<span style='flex:1;color:#e0e0e0;font-size:11px;'>{label}</span>"
                    f"<span style='color:#fff;font-size:12px;font-weight:700;margin-left:8px;'>{usd_str}</span>"
                    f"</div>"
                    f"<div style='display:flex;justify-content:space-between;margin-top:3px;'>"
                    f"{d_badge}"
                    f"<span style='color:#5a6370;font-size:10px;'>{time_str}</span>"
                    f"</div></div>"
                )
            st.markdown(
                f"<div style='background:#161a1e;border:1px solid #2b3139;border-radius:8px;padding:8px 12px;'>"
                f"{whale_html}</div>",
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                "<div style='color:#5a6370;font-size:12px;padding:12px;background:#161a1e;"
                "border:1px solid #2b3139;border-radius:8px;'>No large BTC movements detected.</div>",
                unsafe_allow_html=True,
            )
    ...
    with ic2:
        st.markdown(
            "<div class='card-title' style='font-size:12px;'>📰 Last 5 BTC Headlines (Claude-classified)</div>",
            unsafe_allow_html=True,
        )
        if news_feed:
            news_html = ""
            for n in news_feed:
                ...
                s_col   = "#0ecb81" if sent == "bullish" else ("#f6465d" if sent == "bearish" else "#848e9c")
                title   = n.get("title", "—")
                ...
                news_html += (
                    ...
                    f"<span style='font-size:11px;margin-top:1px;'>{icon}</span>"
                    f"<span style='color:#e0e0e0;font-size:11px;line-height:1.4;flex:1;'>{title}</span>"
                    f"</div>"
                    f"<div style='display:flex;justify-content:space-between;margin-top:4px;padding-left:17px;'>"
                    f"<span style='color:#848e9c;font-size:10px;'>{source}</span>"
                    f"<span style='color:{s_col};font-size:10px;font-weight:700;'>{sent.upper()} {conf_pct}</span>"
                    f"</div></div>"
                )
            st.markdown(
                f"<div style='background:#161a1e;border:1px solid #2b3139;border-radius:8px;padding:8px 12px;'>"
            ...
            st.markdown(
                "<div style='color:#5a6370;font-size:12px;padding:12px;background:#161a1e;"
                "border:1px solid #2b3139;border-radius:8px;'>No credible BTC headlines available.</div>",
                unsafe_allow_html=True,
            )

    # ── Column 3: Merged Signal ───────────────────────────────────────────────
    with ic3:
        st.markdown(
            "<div class='card-title' style='font-size:12px;'>⚡ Merged Signal (Alt Data)</div>",
            unsafe_allow_html=True,
        )
        ...
        weight_rows = "".join([
            ...
            f"<span style='color:#848e9c;font-size:10px;'>{k.capitalize()}</span>"
            f"<span style='color:#e0e0e0;font-size:10px;font-weight:600;'>{int(v*100)}%</span>"
            f"</div>"
            for k, v in WEIGHTS.items()
        ])
        st.markdown(
            f"<div style='background:{sig_bg};border:1px solid {sig_color}44;border-radius:10px;padding:14px 16px;'>"
            # Signal badge
            f"<div style='text-align:center;margin-bottom:10px;'>"
            f"<span style='color:{sig_color};font-size:2rem;font-weight:900;letter-spacing:.06em;'>{signal}</span>"
            f"<div style='color:#fff;font-size:1.1rem;font-weight:700;margin-top:2px;'>{conf_pct}% confidence</div>"
            f"</div>"
            # Confidence bar
            f"<div style='background:#2b3139;border-radius:4px;height:6px;margin-bottom:12px;'>"
            f"<div style='background:{sig_color};width:{bar_w}%;height:6px;border-radius:4px;'></div>"
            f"</div>"
            # Reasoning
            f"<div style='color:#e0e0e0;font-size:11px;line-height:1.5;background:#161a1e;"
            f"border-radius:6px;padding:8px;margin-bottom:12px;border-left:3px solid {sig_color};'>"
            f"{reasoning}</div>"
            # Weight breakdown
            f"<div style='border-top:1px solid #2b3139;padding-top:10px;'>"
            f"<div style='color:#848e9c;font-size:10px;font-weight:600;margin-bottom:6px;'>SIGNAL WEIGHTS</div>"
            f"{weight_rows}"
            f"</div>"
            f"</div>",
            unsafe_allow_html=True,
...
with tab_terminal:
    ...
    with col_tr:
        ...
        def render_trades_and_ai():
            ...
            # ── Price signal (EMA / RSI / order book) ────────────────────────
            depth  = api_depth(sym, 14)
            klines = api_klines(sym, "5m", 80)
            price_signal, price_conf_raw, reasons = "HOLD", 0.5, []
            ...
            if klines and len(klines) > 20 and depth:
                ...
                bid_pct = bid_vol / (bid_vol + ask_vol or 1)
                ...
                raw_conf = min(100, max(0, 50 + scores))
                price_conf_raw = raw_conf / 100.0
                if   raw_conf >= 65: price_signal = "BUY"
                elif raw_conf <= 35: price_signal = "SELL"
                else:                price_signal = "HOLD"

            # ── Merge with alt data (whale + news) if available ───────────────
            agg      = st.session_state.alt_aggregator
            merged   = agg.combine_with_price_signal(price_signal, price_conf_raw)
            signal   = merged["signal"]
            conf     = round(merged["confidence"] * 100)
            alt_note = merged.get("reasoning", "")

            sig_color = {"BUY": "#0ecb81", "SELL": "#f6465d", "STRONG BUY": "#0ecb81", "STRONG SELL": "#f6465d"}.get(signal, "#848e9c")

            # Show merged nudge bar
            label = "MERGED SIGNAL" if agg._last_result else "PRICE SIGNAL"
            st.markdown(
                f"<div style='background:#1f2933;border:1px solid #2b3139;border-radius:6px;"
                f"padding:6px 12px;display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;'>"
                f"<span style='color:#848e9c;font-size:11px;font-weight:600;'>{label}:</span>"
                f"<span style='color:{sig_color};font-weight:900;font-size:14px;letter-spacing:1px;'>{signal}</span>"
                f"<span style='color:#fff;font-size:12px;font-weight:700;'>{conf}% Conf</span>"
                f"</div>",
                unsafe_allow_html=True,
            )

            st.markdown(
                "<div class='card-title' style='font-size:13px;margin-bottom:8px;'>🧠 AI Engine Reasoning</div>",
                unsafe_allow_html=True,
            )
            reason_html = "".join([
                f"<div style='font-size:11px;color:#e0e0e0;margin-bottom:4px;"
                f"padding-left:8px;border-left:2px solid {sig_color};'>{r}</div>"
                for r in reasons
            ])
            if alt_note:
                reason_html += (
                    f"<div style='font-size:11px;color:#a0a8b0;margin-top:6px;padding:6px 8px;"
                    f"background:#0d1117;border-radius:4px;border-left:2px solid #f0b429;'>"
                    f"🔬 {alt_note}</div>"
                )
            st.markdown(
                f"<div style='background:#161a1e;border:1px solid #2b3139;border-radius:6px;"
                f"padding:10px;margin-bottom:20px;'>{reason_html}</div>",
                unsafe_allow_html=True,
            )
