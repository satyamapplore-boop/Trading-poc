...
import asyncio
from alternative_data_signal import AlternativeDataAggregator, ECONOMIC_EVENT_WINDOW_MINS
...
def api_alternative_signal():
    try:
        agg = AlternativeDataAggregator()
        return asyncio.run(agg.get_signal())
    except Exception as e:
        print("Alternative signal error:", e)
        return None
    ...
    with col_tr:
        ...
        def render_trades_and_ai():
            ...
            if klines and len(klines) > 20 and depth:
                ...
                alt_sig_data = api_alternative_signal()
                if alt_sig_data:
                    agg = AlternativeDataAggregator()
                    agg._last_result = alt_sig_data
                    
                    base_price_sig = "HOLD"
                    if "BUY" in signal: base_price_sig = "BUY"
                    elif "SELL" in signal: base_price_sig = "SELL"
                    
                    final = agg.combine_with_price_signal(base_price_sig, conf / 100.0)
                    
                    final_sig = final["signal"]
                    final_conf = int(final["confidence"] * 100)
                    
                    if final_sig == "BUY" and final_conf >= 65: signal, sig_color = "STRONG BUY", "#0ecb81"
                    elif final_sig == "SELL" and final_conf >= 65: signal, sig_color = "STRONG SELL", "#f6465d"
                    elif final_sig == "BUY": signal, sig_color = "BUY", "#0ecb81"
                    elif final_sig == "SELL": signal, sig_color = "SELL", "#f6465d"
                    else: signal, sig_color = "HOLD", "#848e9c"
                    
                    conf = final_conf
                    reasons.append(f"Alt Data: {final['reasoning']}")
                else:
                    reasons.append("Alt Data: Scanning / Unavailable...")
            ...
            st.markdown(f"<div style='background:#161a1e; border:1px solid #2b3139; border-radius:6px; padding:10px; margin-bottom:12px;'>{reason_html}</div>", unsafe_allow_html=True)

            # ── Inline Alt Data Detail Expanders ──
            if alt_sig_data:
                raw = alt_sig_data.get("raw", {})
                news_raw = raw.get("news", {})
                whale_raw = raw.get("whale", {})
                eco_raw = raw.get("economic", {})

                with st.expander(f"📰 News Headlines  —  Score: {alt_sig_data.get('news_score', 0.5):.0%}", expanded=False):
                    headlines = news_raw.get("headlines", [])
                    classified = news_raw.get("classified", [])
                    if headlines:
                        class_map = {c.get('index', 0): c for c in classified}
                        news_html = ""
                        for i, h in enumerate(headlines[:10]):
                            title = h.get('title', '')
                            src = h.get('source', 'Unknown')
                            cl = class_map.get(i + 1, {})
                            sent = cl.get('sentiment', 'neutral')
                            sc = cl.get('score', 0.5)
                            if sent == 'bullish':
                                badge = f"<span style='background:#0ecb8120;color:#0ecb81;padding:2px 6px;border-radius:3px;font-size:9px;font-weight:700;'>▲ BULL {sc:.0%}</span>"
                            elif sent == 'bearish':
                                badge = f"<span style='background:#f6465d20;color:#f6465d;padding:2px 6px;border-radius:3px;font-size:9px;font-weight:700;'>▼ BEAR {sc:.0%}</span>"
                            else:
                                badge = f"<span style='background:#f0b90b20;color:#f0b90b;padding:2px 6px;border-radius:3px;font-size:9px;font-weight:700;'>● NEUT</span>"
                            url = h.get('url', '')
                            link = f" <a href='{url}' target='_blank' style='color:#627eea;font-size:9px;'>↗</a>" if url else ""
                            news_html += f"<div style='padding:5px 0;border-bottom:1px solid #1f2933;font-size:11px;color:#e0e0e0;'>{badge} <span style='color:#848e9c;'>[{src}]</span> {title}{link}</div>"
                        st.markdown(news_html, unsafe_allow_html=True)
                    else:
                        st.caption("No headlines available")

                with st.expander(f"🐋 Whale Transfers  —  Score: {alt_sig_data.get('whale_score', 0.5):.0%}", expanded=False):
                    transfers = whale_raw.get('transfers', [])
                    net_sig = whale_raw.get('net_signal', 'neutral')
                    w_col = '#0ecb81' if net_sig == 'bullish' else '#f6465d' if net_sig == 'bearish' else '#f0b90b'
                    whale_source = whale_raw.get('source', 'Blockchain.com Live')
                    st.markdown(f"<div style='font-size:11px;color:{w_col};font-weight:700;margin-bottom:6px;'>Aggregate Flow: {net_sig.upper()} ({whale_source})</div>", unsafe_allow_html=True)
                    if transfers:
                        wh_html = ""
                        for t in transfers[:8]:
                            cur = t.get('currency', 'BTC')
                            amt_btc = t.get('amount', 0)
                            amt_usd = t.get('amount_usd', 0)
                            d = t.get('direction', 'neutral')
                            tx_hash = t.get('hash', '...')
                            dc = '#0ecb81' if d == 'bullish' else '#f6465d' if d == 'bearish' else '#848e9c'
                            di = '↗' if d == 'bullish' else '↘' if d == 'bearish' else '↔'
                            wh_html += f"<div style='display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px solid #1a1f26;font-size:10px;font-family:monospace;'><span style='color:#f7931a;width:35px;'>{cur}</span><span style='color:#e0e0e0;width:90px;text-align:right;'>{amt_btc:,.1f} (${amt_usd / 1e6:,.1f}M)</span><span style='color:{dc};width:55px;text-align:center;font-weight:700;'>{di}</span><span style='color:#5a6370;flex:1;text-align:right;'>Hash: {tx_hash}</span></div>"
                        st.markdown(wh_html, unsafe_allow_html=True)
                    else:
                        st.caption("No recent large transfers")

                with st.expander(f"📅 Economic Events & Policy —  Score: {alt_sig_data.get('economic_score', 0.5):.0%}", expanded=False):
                    macro = eco_raw.get('macro_events', [])
                    policy = eco_raw.get('policy_news', [])
                    
                    if macro:
                        st.markdown("<div style='font-size:11px;color:#f0b90b;font-weight:700;margin-top:5px;'>🌍 High Impact Macro (24h)</div>", unsafe_allow_html=True)
                        for ev in macro:
                            st.markdown(f"<div style='font-size:10px;color:#e0e0e0;padding:2px 0;'>⏱ {ev.get('time','')} — {ev.get('title','')} (in {ev.get('hours_away',0)}h)</div>", unsafe_allow_html=True)
                    
                    if policy:
                        st.markdown("<div style='font-size:11px;color:#627eea;font-weight:700;margin-top:10px;'>⚖️ Adoption & Policy Alerts</div>", unsafe_allow_html=True)
                        for p in policy[:5]:
                            st.markdown(f"<div style='font-size:10px;color:#e0e0e0;padding:2px 0;'>🔹 {p.get('title','')}</div>", unsafe_allow_html=True)
                    
                    if not macro and not policy:
                        st.markdown("<div style='font-size:11px;color:#0ecb81;'>✅ No imminent high-impact macro or policy shifts</div>", unsafe_allow_html=True)
